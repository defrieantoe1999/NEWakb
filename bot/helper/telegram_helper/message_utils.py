...
def update_all_messages():
    ...
    if msg is None:
        return
    msg += f"<b>🖥️CPU:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>📝RAM::</b> <code>{psutil.virtual_memory().percent}%</code>" \
           f" <b>📀Penyimpanan:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        ...
        msg += f"\n<b>FREE🔗:</b> <code>{free}</code> | <b>⏰UPTIME:</b> <code>{currentTime}</code>\n<b>Kecepatan unduh:</b> <code>{dlspeed}/s</code> 🔻 | <b>Kecepatan Upload:</b> <code>{ulspeed}/s</code> 🔺\n"
    with status_reply_dict_lock:
...
def sendStatusMessage(msg, bot):
    ...
    if progress is None:
        progress, buttons = get_readable_message()
    progress += f"<b>🖥️CPU:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>📝RAM:</b> <code>{psutil.virtual_memory().percent}%</code>" \
